...
@Import({ItemMappingImpl.class, CommentMappingImpl.class, TestSecurityConfig.class})
public class ItemControllerTest {
    ...
    @BeforeEach
    public void setUp() {
        owner = User.builder().id(1L).name("user1").email("user1@mail.ru").password("1234").build();
        user = User.builder().id(2L).name("user1").email("user1@mail.ru").password("1234").build();
    ...
    public MockHttpServletResponse performGetItem(long itemId, long userId) throws Exception {
        MvcResult result = mvc.perform(get("/items/me/{itemId}", itemId)
    ...
    public MockHttpServletResponse performGetUserItems(long userId) throws Exception {
        MvcResult result = mvc.perform(get("/items/me")
                        .header(USER_ID_HEADER, userId)
                        .characterEncoding(StandardCharsets.UTF_8)
                        .contentType(MediaType.APPLICATION_JSON)
                        .accept(MediaType.APPLICATION_JSON))
