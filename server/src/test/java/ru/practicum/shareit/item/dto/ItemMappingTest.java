...
public class ItemMappingTest {
    ...
    public void toDtoTest() {
        ...
        Item item = new Item(5L, "item name", "desc", true, user, new ItemRequest());
        dto = itemMapping.toDto(item);
        assertThat(dto, is(notNullValue()));
        assertThat(dto.getId(), is(5L));
        assertThat(dto.getRequestId(), is(nullValue()));

        item = new Item(5L, "item name", "desc", true, user, new ItemRequest(1L, "desc", user, LocalDateTime.now()));
        dto = itemMapping.toDto(item);
        assertThat(dto, is(notNullValue()));
        assertThat(dto.getId(), is(5L));
    ...
    public void toRequestDtoTest() {
        ...
        Item item = new Item(5L, "item name", "desc", true, null, new ItemRequest());
        dto = itemMapping.toRequestDto(item);
        assertThat(dto, is(notNullValue()));
        assertThat(dto.getId(), is(item.getId()));
        assertThat(dto.getOwnerId(), is(nullValue()));

        item = new Item(5L, "item name", "desc", true, new User(), new ItemRequest());
        dto = itemMapping.toRequestDto(item);
        assertThat(dto, is(notNullValue()));
        assertThat(dto.getOwnerId(), is(nullValue()));
    }
