...
public class BookingService {
    ...
    public List<Booking> getUserBookings(Long userId, State state) {
        ...
        switch (state) {
            ...
            case FUTURE:
                return repository.getUserBookingsFuture(userId, Status.APPROVED, Status.WAITING, LocalDateTime.now());
            case WAITING:
                return repository.findAllByBookerIdAndStatusOrderByStartDesc(userId, Status.WAITING);
            case REJECTED:
                return repository.getUserBookingsRejected(userId, Status.REJECTED, Status.CANCELED);
            default:
    ...
    public List<Booking> getOwnerBookings(Long ownerId, State state) {
        ...
        log.debug("Получен список бронирований со статусом {} для вещей владельца с id {}", state, ownerId);
        switch (state) {
            case ALL:
                return repository.getOwnerBookingsAll(ownerId);
            case CURRENT:
                return repository.getOwnerBookingsCurrent(ownerId, LocalDateTime.now(), LocalDateTime.now());
            case PAST:
                return repository.getOwnerBookingsPast(ownerId, Status.APPROVED, LocalDateTime.now());
            case FUTURE:
                return repository.getOwnerBookingsFuture(ownerId, Status.APPROVED, Status.WAITING, LocalDateTime.now());
            case WAITING:
                return repository.getOwnerBookingsWaiting(ownerId, Status.WAITING);
            case REJECTED:
                return repository.getOwnerBookingsRejected(ownerId, Status.REJECTED, Status.CANCELED);
            default:
                return new ArrayList<>();
        }
    }
    ...
    public Map<Long, BookingShort> getNextBooking(List<Long> itemsId) {
        List<BookingShort> bookings = repository.getNextBookings(itemsId, Status.APPROVED, LocalDateTime.now());
        return bookings.stream()
                .collect(Collectors.toMap(BookingShort::getItemId, Function.identity()));
    }
